...
class GraphModel2(nn.Module):
    def __init__(self, g, n_layers, input_size, hidden_size, output_size, nonlinearity):
        ...
        self.layers.append(GATConv(input_size, hidden_size, num_heads, activation=nonlinearity, residual=True))
        for _ in range(n_layers - 1):
            self.layers.append(GATConv(hidden_size*num_heads, hidden_size, num_heads, activation=nonlinearity, residual=True))
        self.layers.append(GATConv(hidden_size*num_heads, output_size, num_heads, residual=True))
...
def main(args):
    ...
    model = BasicGraphModel(g=train_dataset.graph, n_layers=4, input_size=n_features,
                            hidden_size=256, output_size=n_classes, nonlinearity=F.elu).to(device)

    # model = GraphModel2(g=train_dataset.graph, n_layers=2, input_size=n_features,
    #                         hidden_size=512, output_size=n_classes, nonlinearity=F.elu).to(device)
